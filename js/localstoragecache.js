var isLocalStorageSupported = function () {
  var ilss = "ilss";
  try {
    localStorage.setItem(ilss, ilss);
    localStorage.removeItem(ilss);
    return true;
  } catch (e) {
    return false;
  }
...
//
// CHANGES:
//  - removed Modernizr dependency (localstorage test)
//    Jon LaBelle, Mon Mar 04 2013 22:24:41 GMT-0600 (CST)
//
$.ajaxPrefilter(function (options, originalOptions, jqXHR) {
 
  // Cache it ?
  if (!isLocalStorageSupported || !options.localCache) return;
  ...
  if (value) {
    ...
    //If it not in the cache, we change the success callback, just put data
    //on localstorage and after that apply the initial callback
    if (options.success) {
